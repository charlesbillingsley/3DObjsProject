/**
 * Created by christopherfracassi on 2/19/17.
 */
...
class Sphere {
     ...
     */
    constructor (gl, radius, stacks, col1, col2) {
        ...
        for (let k = 0; k <= stacks; k++) {
            let angle = k * Math.PI / stacks;
            let sin = Math.sin(angle);
            let cos =  Math.cos(angle);

            for(let i = 0; i <= stacks; i++){
                let angle2 = i * 2 * Math.PI / stacks;
        ...
        for (let k = 0; k < stacks; k++){
            for(let i = 0; i < stacks; i++){
                let first = (k * (stacks + 1)) + i;
                let second = first + stacks + 1;

                topIndex.push(first);
                topIndex.push(second);
    ...
    draw(vertexAttr, normAttr, modelUniform, coordFrame) {
        ...
        gl.vertexAttribPointer(normAttr, 3, gl.FLOAT, false, 24 , 12);
